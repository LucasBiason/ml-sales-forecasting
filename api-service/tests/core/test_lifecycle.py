...
from app.core.lifecycle import startup_event, shutdown_event, forecaster
...
async def test_startup_event_success():
    ...
    with patch.object(forecaster, 'load') as mock_load:
        with patch.object(forecaster, 'get_model_info', return_value={"loaded": True, "model_type": "RandomForest"}):
            await startup_event()
